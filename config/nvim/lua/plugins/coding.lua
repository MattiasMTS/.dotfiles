return {
  ...
  -- },
  {
    ...
    dependencies = {
      ...
      { "hrsh7th/cmp-emoji", enabled = true },
      { "hrsh7th/cmp-nvim-lua" },
      -- disable these for now, not using snippets
    ...
    opts = function(_, opts)
      ...
      local sources = {
        { name = "cmp-dbt", filetypes = "sql" },
        -- { name = "copilot" },
        { name = "cmp-dbee", filetypes = "sql" },
        { name = "emoji" },
        { name = "buffer" },
      }
      ...
      opts.formatting = vim.tbl_deep_extend("force", opts.formatting, {
        format = function(entry, vim_item)
          local default_kind = require("lspkind").presets.default[vim_item.kind]
          if default_kind then
            vim_item.kind = default_kind .. " " .. vim_item.kind
          end
          vim_item.menu = ({
            -- copilot = "[AI]",
            nvim_lsp = "[LSP]",
      ...
      -- remove ghost_text for copilot cmp
      opts.experimental = {
        ghost_text = false,
      }
    end,
